...
__kernel void mat_mult_tile_local_vector (
    ...
    // A_star is of size (N0_C, N1_A_star), (i0, n)
    // B_star is of size (N1_A_star*8, N1_C), (n, i1)
    // C is of size (N0_C, N1_C), (i0, i1)
    ...
    __local float8* shared_B_star_s1 = &shared_B_star[s1*chunk_len];
    ...
    for (int chunk_id=start_chunk_id; chunk_id<end_chunk_id; chunk_id++) {
        ...
        // Starting positions for the copy
        __global float8* A_star_i0 = &A_star[i0*N1_A_star+chunk_id*chunk_len];
        __global float* B_star_i1 = &B_star[chunk_id*chunk_len*8*N1_C+i1];
        ...
        // From column i1 of B_star, this is not right
        for (int n = start0; n<end0; n++) {
            shared_B_star_s1[n].s0 = B_star_i1[(0+n*8)*N1_C];
            shared_B_star_s1[n].s1 = B_star_i1[(1+n*8)*N1_C];
            shared_B_star_s1[n].s2 = B_star_i1[(2+n*8)*N1_C];
            shared_B_star_s1[n].s3 = B_star_i1[(3+n*8)*N1_C];
            shared_B_star_s1[n].s4 = B_star_i1[(4+n*8)*N1_C];
            shared_B_star_s1[n].s5 = B_star_i1[(5+n*8)*N1_C];
            shared_B_star_s1[n].s6 = B_star_i1[(6+n*8)*N1_C];
            shared_B_star_s1[n].s7 = B_star_i1[(7+n*8)*N1_C];
        }
        ...
        for (size_t n=0; n<chunk_len; n++) {
            ...
            // Loop across row i0 of A
            // and down column i1 of B
            temp1+=shared_A_star_s0[n]*shared_B_star_s1[n];
        }
