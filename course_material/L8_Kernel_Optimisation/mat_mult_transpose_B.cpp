...
int main(int argc, char** argv) {
    ...
    // Desired global_size
    const size_t global_size_transp[]={ N1_C, N1_A };
    ...
    H_ERRCHK(clSetKernelArg(kernel_transp, 0, sizeof(cl_mem), &B_d ));
    H_ERRCHK(clSetKernelArg(kernel_transp, 1, sizeof(cl_mem), &B_dT ));
    H_ERRCHK(clSetKernelArg(kernel_transp, 2, sizeof(cl_uint), &N1_A ));
    ...
    cl_kernel kernel_mat_mult=clCreateKernel(
            program, 
            "mat_mult_transpose_B", 
            &errcode
    );
    H_ERRCHK(errcode);
        
    // Set arguments to the kernel (not thread safe)
    H_ERRCHK(clSetKernelArg(kernel_mat_mult, 0, sizeof(cl_mem), &A_d));
    H_ERRCHK(clSetKernelArg(kernel_mat_mult, 1, sizeof(cl_mem), &B_dT));
    ...
    H_ERRCHK(clReleaseMemObject(B_dT));
