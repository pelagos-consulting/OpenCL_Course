...
int main(int argc, char** argv) {
    ...
    // AT_star is of size (N1_A_star, N0_C)
    // B_star is of size (N1_A_star, N1_C)
    // C is of size (N0_C, N1_C)
    
    //// Step 5. Prepare OpenCL Buffers for matrices A, B, and C ////

    // Make A_star_d, AT_star_d, and B_star_d - the enlarged matrices
    ...
    // Create buffer AT_star_d
    cl_mem AT_star_d = clCreateBuffer(
        context, 
        CL_MEM_READ_WRITE, 
        nbytes_A_star, 
        NULL, 
    ...
    // Desired global_size
    size_t global_size_transp[]={ N1_A_star, N0_C };
    ...
    H_ERRCHK(clSetKernelArg(kernel_transp, 0, sizeof(cl_mem), &A_star_d ));
    H_ERRCHK(clSetKernelArg(kernel_transp, 1, sizeof(cl_mem), &AT_star_d ));
    H_ERRCHK(clSetKernelArg(kernel_transp, 2, sizeof(cl_uint), &N0_C ));
    H_ERRCHK(clSetKernelArg(kernel_transp, 3, sizeof(cl_uint), &N1_A_star ));
    ...
    H_ERRCHK(clSetKernelArg(kernel_mat_mult, 0, sizeof(cl_mem), &AT_star_d ));
    ...
    H_ERRCHK(clSetKernelArg(kernel_mat_mult, 3, sizeof(cl_uint), &N1_A_star ));
    ...
    H_ERRCHK(clSetKernelArg(kernel_mat_mult, 7, sizeof(cl_uint), &start_chunk_id ));
    H_ERRCHK(clSetKernelArg(kernel_mat_mult, 8, sizeof(cl_uint), &end_chunk_id ));
    ...
    H_ERRCHK(clReleaseMemObject(AT_star_d));
