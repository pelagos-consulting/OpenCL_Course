...
int main(int argc, char** argv) {
    ...
    // Allocate SVM memory for matrix F using a fine-grained buffer
    cl_float *F_svm = (cl_float*)clSVMAlloc(
        context,
    ...
    // The memory allocation will return F_svm == NULL if allocation failed
    // enforce a check
    assert(F_svm!=NULL);
    ...
    //// Step 5. Allocate OpenCL Buffers for matrices D and E ////

    // Make ordinary OpenCL buffers for matrices D, and E
    ...
    H_ERRCHK(clSetKernelArgSVMPointer(kernel, 2, F_svm));
    ...
    // At this point F_svm is available for access by the host
    ...
    // Compute the residual between F_svm and F_answer_h
    m_residual(F_answer_h, F_svm, F_residual_h, N0_F, N1_F);

    // Pretty print the output matrices
    std::cout << "The output array F_svm (as computed with OpenCL) is\n";
    m_show_matrix(F_svm, N0_F, N1_F);

    std::cout << "The CPU solution (F_answer_h) is \n";
    m_show_matrix(F_answer_h, N0_F, N1_F);
    
    std::cout << "The residual (F_answer_h-F_svm) is\n";
    m_show_matrix(F_residual_h, N0_F, N1_F);

    // Print the maximum error between matrices
    float max_err = m_max_error(F_svm, F_answer_h, N0_F, N1_F);

    // Write out the arrays to file
    h_write_binary(D_h, "array_D.dat", nbytes_D);
    h_write_binary(E_h, "array_E.dat", nbytes_E);
    h_write_binary(F_svm, "array_F.dat", nbytes_F);
    ...
    // Free the SVM buffer F_svm
    clSVMFree(context, F_svm);
