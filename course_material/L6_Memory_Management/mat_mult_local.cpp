...
int main(int argc, char** argv) {
    ...
    H_ERRCHK(clSetKernelArg(kernel, 3, local_size[1]*N1_A*sizeof(cl_float), NULL ));
    // Local size is going to be (local_size[0], N1_A)
    H_ERRCHK(clSetKernelArg(kernel, 4, local_size[0]*N1_A*sizeof(cl_float), NULL ));
    H_ERRCHK(clSetKernelArg(kernel, 5, sizeof(cl_uint), &N1_A ));
    H_ERRCHK(clSetKernelArg(kernel, 6, sizeof(cl_uint), &N0_C ));
    H_ERRCHK(clSetKernelArg(kernel, 7, sizeof(cl_uint), &N1_C ));
